...
if "ocr_results" not in st.session_state:
    st.session_state.ocr_results = None      # {"unique_texts": [...], "frames": [...]}
if "gemini_result" not in st.session_state:
    st.session_state.gemini_result = None    # str (markdown)
...
if st.session_state.analyzed:
            ...
            if st.button("화면 텍스트 분석 시작", key="ocr_btn"):
                with tempfile.TemporaryDirectory() as tmpdir:
                    ...
                    for i, frame_data in enumerate(frames):
                        ...
                        if texts:
                            # PIL Image → bytes로 직렬화하여 session_state에 저장 가능하게
                            buf = io.BytesIO()
                ...
                st.session_state.ocr_results = {
                    ...
                    "frames": frame_display_data,
                }
                st.rerun()

        else:
            ocr = st.session_state.ocr_results
            unique_texts = ocr["unique_texts"]
            frame_display_data = ocr["frames"]
    ...
    with tab4:
        st.markdown("""
**Gemini가 실제 영상 파일을 직접 분석합니다.**

유튜브 시스템 자막이 아닌, 편집으로 삽입된 자막 · 화면 전체 구성 · 스토리 흐름 · 바이럴 요소를 분석합니다.
        """)
        ...
        if not api_key:
            st.error(
                ...
                "Streamlit Cloud의 Secrets에서 `GEMINI_API_KEY`를 설정해주세요."
            )
        elif st.session_state.gemini_result is None:
            st.info("영상을 다운로드하고 Gemini에 업로드합니다. 영상 길이에 따라 1~3분 소요될 수 있습니다.")
            ...
            if st.button("AI 영상 분석 시작", key="gemini_btn", type="primary"):
                with tempfile.TemporaryDirectory() as tmpdir:
                    ...
                    with st.spinner("Gemini에 영상 업로드 중..."):
                        try:
